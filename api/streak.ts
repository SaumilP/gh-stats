...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (format === "json") {
      ...
      if (cache) {
        await cacheSet(cache, key, body, ttl, staleExtraSecondsFor("streak"));
      }
    ...
    const svg = renderStreak(style, username, streak, { compact });

    if (cache) {
      await cacheSet(cache, key, svg, ttl, staleExtraSecondsFor("streak"));
    }
    ...
    sendSvg(
      ...
      renderErrorCard(style, {
        ...
        username,
        requestId,
        title: "Failed to generate streak card",
        hint: "Try again later",
        detail,
        compact,
      }),
