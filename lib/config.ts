...
export function ttlSecondsFor(endpoint: "stats" | "repos" | "langs" | "streak" | "pin" | "gist" | "wakatime" | "focus" | "impact") {
  ...
  if (endpoint === "focus") return envInt("CACHE_TTL_FOCUS", 86400);
  if (endpoint === "impact") return envInt("CACHE_TTL_IMPACT", 86400);
  return envInt("CACHE_TTL_STATS", 21600);
}

export function staleExtraSecondsFor(endpoint: "stats" | "repos" | "langs" | "streak" | "pin" | "gist" | "wakatime" | "focus" | "impact") {
  // Keep stale entries around to enable soft-TTL checks without recomputing.
  if (endpoint === "langs") return 7 * 86400;
  if (endpoint === "focus") return 7 * 86400;
  if (endpoint === "impact") return 7 * 86400;
  return 2 * 86400;
}
